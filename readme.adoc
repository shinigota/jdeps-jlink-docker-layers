...
* Le build multi stage et les layers Spring reposent en grande partie sur les layers Docker :
** Le poids total de l'image est réduit
** Les layers sont réutilisés
** *La taille du cache de build Docker augmente de manière conséquente*

Il existe un https://docs.docker.com/build/cache/garbage-collection/[Garbage Collector pour le cache]
